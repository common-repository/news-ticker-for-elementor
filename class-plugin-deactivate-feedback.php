...
if( ! class_exists( 'WBELNT_Usage_Feedback') ) {
	
	class WBELNT_Usage_Feedback {
		
		private $wbelnt_version = '0.9.3';
		private $home_url = '';
		private $plugin_file = '';
		private $plugin_name = '';
		...
		public function init() {
			...
			add_action( 'wp_ajax_wbelnt_goodbye_form', array( $this, 'goodbye_form_callback' ) );
		...
		public function admin_enqueue_scripts(){
			...
			$css='';
			$css .= '.button.qcwbelnt-promo-link {
		      color: #ff0000;
		      font-weight: normal;
		      margin-left: 0;
			...
			wp_add_inline_style( 'wb-nt-admin-style', $css );
		}
		...
		public function send_data( $body ) {
			...
			foreach($body as $key=>$value){
			    ...
			    $title   = esc_html('News Ticker for Elementor Plugin Deactivation Notice');
				$headers = array('From: Elementor News Ticker <mailer@just-a-fake-from-address.com>');
		...
		public function deactivate_this_plugin() {
			...
			if( false !== get_option( 'wbelnt_deactivation_reason_' . $this->plugin_name ) ) {
				$body['deactivation_reason'] = get_option( 'wbelnt_deactivation_reason_' . $this->plugin_name );
				delete_option('wbelnt_deactivation_reason_' . $this->plugin_name);
			}
			if( false !== get_option( 'wbelnt_deactivation_details_' . $this->plugin_name ) ) {
				$body['deactivation_details'] = get_option( 'wbelnt_deactivation_details_' . $this->plugin_name );
				delete_option('wbelnt_deactivation_details_' . $this->plugin_name);
			}

			if( false !== get_option( 'wbelnt_deactivation_email_' . $this->plugin_name ) ) {
				$body['deactivation_email'] = get_option( 'wbelnt_deactivation_email_' . $this->plugin_name );
				delete_option('wbelnt_deactivation_email_' . $this->plugin_name);
			}

			if( false !== get_option( 'wbelnt_deactivation_main_reason_' . $this->plugin_name ) ) {
				$body['deactivation_main_reason'] = get_option( 'wbelnt_deactivation_main_reason_' . $this->plugin_name );
				delete_option('wbelnt_deactivation_main_reason_' . $this->plugin_name);
			}
		...
		public function filter_action_links( $links ) {
			...
			if( isset( $links['deactivate'] ) && $this->include_goodbye_form ) {
				...
				$deactivation_link = str_replace( '<a ', '<div class="wpb-goodbye-form-wrapper"><span class="wpb-goodbye-form" id="wpb-goodbye-form-' . esc_attr( $this->plugin_name ) . '"></span></div><a onclick="javascript:event.preventDefault();" id="wpb-goodbye-link-wbelnt-' . esc_attr( $this->plugin_name ) . '" ', $deactivation_link );
		...
		public function form_filterable_text() {
			...
			return apply_filters( 'wbelnt_form_text_' . esc_attr( $this->plugin_name ), $form );
		}
		...
		public function goodbye_ajax() {
			...
			if( is_array( $form['options'] ) ) {
				$html .= '<div class="wbelnt-goodbye-options"><p>';
				foreach( $form['options'] as $option ) {
					$html .= '<input type="radio" name="wbelnt-goodbye-options" id="' . str_replace( " ", "", esc_attr( $option ) ) . '" value="' . esc_attr( $option ) . '"> <label for="' . str_replace( " ", "", esc_attr( $option ) ) . '">' . esc_attr( $option ) . '</label><br>';
				}
				$html .= '</p><div id="wbelnt_additional_content" style="display:none;"><label for="wbelnt-goodbye-reasons">' . esc_html( $form['email'] ) .'</label><br><input type="email" name="wbelnt-goodbye-email" id="wbelnt-goodbye-email" value="'.get_option('admin_email').'" /> (Optional)';
				
				$html .= '<br><label for="wbelnt-goodbye-reasons">' . esc_html( $form['details'] ) .'</label><textarea name="wbelnt-goodbye-reasons" id="wbelnt-goodbye-reasons" rows="2" style="width:100%"></textarea></div>';
				$html .= '</div><!-- .wbelnt-goodbye-options -->';
			}
			...
			?>
			<div class="wbelnt-goodbye-form-bg"></div>


			<script>
				jQuery(document).ready(function($){
					 $(document).on('change', '#wpb-goodbye-form-news-ticker-for-elementor input[type=radio]', function() { 
						if($(this).val()=='Deactivating Temporarily' || $(this).val()=='Upgrading to Pro'){
							$('#wbelnt_additional_content').hide();
						}else{
							$('#wbelnt_additional_content').show();
						}
						
					 });

					$('#wpb-goodbye-link-wbelnt-<?php echo esc_attr( $this->plugin_name ); ?>').on('click',function(){
						var url = document.getElementById('wpb-goodbye-link-wbelnt-<?php echo esc_attr( $this->plugin_name ); ?>');
						...
						$('#wpb-submit-form').on('click', function(e){
							...
							$.each($('input[name=\'wbelnt-goodbye-options[]\']:checked'), function(){
								values.push($(this).val());
							});
							var email = $('#wbelnt-goodbye-email').val();
							var details = $('#wbelnt-goodbye-reasons').val();
							var deactivate_main_reason = $('input[name="wbelnt-goodbye-options"]:checked').val();
							var data = {
								'action': 'wbelnt_goodbye_form',
								'values': values,
								'main_reason': deactivate_main_reason,
								'details': details,
								'email': email,
								'security': '<?php echo wp_create_nonce ( 'wbelnt_goodbye_form' ); ?>',
								'dataType': 'json'
							}
						...
						$('.wbelnt-goodbye-form-bg').on('click',function(){
		...
		public function goodbye_form_callback() {
			check_ajax_referer( 'wbelnt_goodbye_form', 'security' );
			if( isset( $_POST['values'] ) ) {
				$values = json_encode( wp_unslash( $_POST['values'] ) );
				update_option( 'wbelnt_deactivation_reason_' . $this->plugin_name, $values );
			}
			if( isset( $_POST['details'] ) ) {
				$details = sanitize_text_field( $_POST['details'] );
				update_option( 'wbelnt_deactivation_details_' . $this->plugin_name, $details );
			}

			if( isset( $_POST['email'] ) ) {
				$email = sanitize_text_field( $_POST['email'] );
				update_option( 'wbelnt_deactivation_email_' . $this->plugin_name, $email );
			}

			if( isset( $_POST['main_reason'] ) ) {
				$main_reason = sanitize_text_field( $_POST['main_reason'] );
				update_option( 'wbelnt_deactivation_main_reason_' . $this->plugin_name, $main_reason );
			}
