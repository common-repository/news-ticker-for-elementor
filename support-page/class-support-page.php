 ...
 *******************************/
add_action('wp_head', 'wbelnt_support_form_ajax_header');

if( !function_exists('wbelnt_support_form_ajax_header') )
{
	function wbelnt_support_form_ajax_header() 
	{
	...
	} //End of wbelnt_support_form_ajax_header
 ...
 *******************************/
add_action( 'wp_ajax_process_wbelnt_promo_form', 'process_wbelnt_promo_form' );

if( !function_exists('process_wbelnt_promo_form') )
{
	function process_wbelnt_promo_form()
	{
 ...
 *******************************/
if( !class_exists('WBELNTSupportPage') ){


	class WBELNTSupportPage{
		...
		function __construct( $plugin_slug = null )
		{
			...
			//add_action( 'wp_ajax_process_wbelnt_promo_form', array(&$this,'process_wbelnt_promo_form') );
			
		} //End of Constructor
		
		function include_promo_page_scripts( $hook )
		{                                 
		   if( $hook == 'news-ticker-for-elementor_page_wb-page-wbelnt-support' ){	   	
		...
		function show_promo_page_callback_func()
		{
			add_submenu_page(
				...
				'manage_options',
				$this->page_slug,
				array(&$this, 'wbelntpromo_support_page_callback_func' )
			);
		 ...
		 *******************************/
		function wbelntpromo_support_page_callback_func()
		{
			
			?>
				
				<div class="wbelnt-support-page-wrapper">
			...
			<?php
		} //End of wbelntpromo_support_page_callback_function
		
		
	
	} //End of the class WBELNTSupportPage
...
//Supply Unique Promo Page Slug as the constructor parameter of the class WBELNTSupportPage. ex: sld-page-2124a to the constructor

//Please create an unique instance for your use, example: $instance_sldf2

$instance = new WBELNTSupportPage('wbelnt-support');

if( is_admin() )
{
	$instance->plugin_menu_slug = "wbel-news-ticker"; //Edit Value
	$instance->plugin_name = "News Ticker for Elementor"; //Edit Value
